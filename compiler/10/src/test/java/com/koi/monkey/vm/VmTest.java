...
public class VmTest {
    ...
    @Test
    @Parameters({
            "let fibonacci = fn(x) { if (x == 0) { return 0; } else {  if (x == 1) {  return 1; } else { fibonacci(x - 1) + fibonacci(x - 2);  } } };fibonacci(15);,610"
    })
    public void testRecursiveFibonacci(String input ,Integer expected){
        runVmTests(input,expected);
    }
