...
public class Vm {
    ...
    public Vm(Bytecode bytecode) {
        ...
        Closure mainClosure = new Closure(mainFn);
        ...
        for (;this.currentFrame().ip < this.currentFrame().instructions().size()-1; ) {
            ...
            switch (op) {
                ...
                case Code.OpGetBuiltin:
                    ...
                    break;
                case Code.OpClosure:
                    constIndex = instructions.get(ip+1);
                    int numFree = instructions.get(ip+2);
                    this.currentFrame().ip+=2;
                    pushClosure(constIndex,numFree);
                    break;
                case Code.OpGetFree:
                    int freeIndex = instructions.get(ip+1);
                    this.currentFrame().ip += 1;
    ...
    private void pushClosure(int constIndex,int numFree) {
        Obj constant = this.constants.get(constIndex);
        CompiledFunction function = (CompiledFunction) constant;
        Obj[] free = new Obj[numFree];
        for (int i = 0; i < numFree; i++) {
            free[i] = this.stack[this.sp - numFree + i];
        }
        this.sp = this.sp - numFree;
        Closure closure = new Closure(function,free);
        push(closure);
    }
    ...
    private void executeCall(int numArgs) {
        ...
        if (callee instanceof Closure) {
            callClosure((Closure)callee,numArgs);
            return;
        }
    ...
    private void callClosure(Closure cl, int numArgs) {
        if (numArgs != cl.fn.numParameters) {
            throw new RuntimeException(String.format("wrong number of arguments: want=%d, got=%d",cl.fn.numParameters,numArgs));
        }
